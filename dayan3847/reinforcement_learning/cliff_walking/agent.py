...
class AgentQLearningGaussian(AgentQLearning):
    def __init__(self, env_: Environment, a: float, s2: float, init_weights_random: bool = True):
        super().__init__(env_)
        self.a = a  # learning rate
        self.s2 = s2  # variance ^ 2
        self.init_weights_random = init_weights_random
        self.q_gaussian_models: list[MultivariateGaussianModel] = self.reset_knowledge()
    ...
    def read_q_value(self, action: int, state=None) -> float:
        if state is None:
            state = self.state
        _m: MultivariateGaussianModel = self.q_gaussian_models[action]
        _r = _m.gi(state)
        return float(_r)
